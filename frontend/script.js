/* ==========================================================
   PREMIUM AQI DASHBOARD – CLEAN FINAL JS (With working suggestions)
========================================================== */
...
const cityList = [
  ...
  "Kolkata","Pune","Dubai","London","New York",
  "Tokyo","Karachi","Hong Kong","Singapore","Sydney"
];
...
function updateSuggestions(value) {
  ...
  matches.forEach(city => {
    ...
    li.addEventListener("mousedown", (e) => {
      e.preventDefault();          // prevents blur from killing click
...
function updateUI(data) {
  const {
    aqi, city, dominantPollutant: dp, coordinates: coords,
    temperature: temp, humidity: hum, time, pollutants
  } = data;
  ...
  temperature.textContent = temp ? temp.toFixed(1) : "--";
  humidity.textContent = hum ? hum.toFixed(1) : "--";
  timeEl.textContent = time || "--";

  const ring = document.querySelector(".ring-progress");
  const circumference = 502;
  ring.style.strokeDashoffset = circumference - (circumference * aqi) / 500;
...
function updatePollutants(p) {
  ...
  Object.entries(pollutants).forEach(([key, val]) => {
    ...
    card.className = "pollutant-card glass";

    card.innerHTML = `
      <div class="poll-name">${key.toUpperCase()}</div>
      <div class="poll-value">${val}</div>
    `;
...
function updateSafetyTips(label) {
  const tips = {
    ...
    Unhealthy: "Wear a mask outdoors and avoid long exposure.",
    "Very Unhealthy": "Avoid going outside unless necessary.",
    Hazardous: "Stay indoors — air is extremely polluted."
  };

  safetyTips.textContent = tips[label] || "";
}
...
function updateHistory(aqi) {
  ...
  const labels = Array.from({ length: aqiHistory.length }, (_, i) =>
    i === aqiHistory.length - 1 ? "Now" : `${i + 1}H`
  );
...
function animateParticles() {
  ...
  ctxP.fillStyle = document.body.classList.contains("dark")
